...
def compute_per_step_distances(probs: list[np.ndarray]) -> list[float]:
    return [float(np.sum(np.abs(probs[i+1] - probs[i]))) for i in range(len(probs) - 1)]
...
def compute_trajectory_alignment(pred_labels: list[str], target_labels: list[str]) -> float:
    if not pred_labels:
        return 0.0
...
def main():
    ...
    for i, conversation in enumerate(conversations):
        ...
        print(f"  Drift: {drift:.4f} | Alignment: {alignment_score:.2f} | "
